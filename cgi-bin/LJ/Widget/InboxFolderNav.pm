...
sub render_body {
    ...
    $u = $remote unless $u;
    my $authas = ( $u == $remote ) ? "" : $u->username;

    my $subfolder_link = sub {
        my ( $link_view, $link_label, $class, $unread, $img ) = @_;
        $class = "" unless $class;
        $unread = "" unless $unread;
        $img = 0 unless $img;

        $class .= " selected" if $opts{view} && $opts{view} eq $link_view;

        my $link = "<a href=.?view=$link_view";
        $link .= "&authas=$authas" if $authas;
        $link .= " class=$class id=esn_folder_$link_view>";
    ...
    my $inbox = $u->notification_inbox
        or return LJ::error_list( BML::ml('inbox.error.couldnt_retrieve_inbox', { 'user' => $u->{user} }) );
    ...
    if ( $authas ) {
         $body .= "<a href=.?authas=" . $authas . " id='esn_folder_all'";
    } else {
        $body .= '<a href="." id="esn_folder_all"';
    }
    $body .= ' class="selected"' unless $opts{view};
    $body .= "><?_ml inbox.menu.all _ml?>$unread_all_html</a>";
    my $messageslabel = $u->is_community ? "inbox.menu.commmessages" : "inbox.menu.messages";
    $body .= $subfolder_link->( "usermsg_recvd", $messageslabel, "subs",
